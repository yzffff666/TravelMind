...
#### T-M2-000b（P0）候选召回 baseline（外部搜索 API + 多路聚合）
- 优先级：P0
- 依赖：T-M2-000a, T-M2-001
- 技术选型依据：`docs/下层能力流水线技术方案.md` 4.2 节
- 主要输出：
  - 通过 `SearchProvider.search()` / `MapProvider.nearby_poi()` 调用外部搜索 API 召回候选
  - Search + Map 多路结果按 `candidate_id`（POI 名 + 城市）去重聚合，保留来源标记
  - 候选池标准字段（`ProviderCandidate`：candidate_id / source / title / snippet / score / tags / extra）
- 验收标准：
  - 候选召回可复现且可配置
  - provider 不可用时有 mock/fixture 降级路径（对齐 design 7.7）
- 建议改动范围：
...
6. `T-M2-001` -> `T-M2-002a` -> `T-M2-002b` -> `T-M2-006` -> `T-M2-000b` -> `T-M2-000c` -> `T-M2-000d` -> `T-M2-003` -> `T-M2-000e` -> `T-M2-004`（`T-M2-005` 后置；技术选型见 `docs/下层能力流水线技术方案.md`）
