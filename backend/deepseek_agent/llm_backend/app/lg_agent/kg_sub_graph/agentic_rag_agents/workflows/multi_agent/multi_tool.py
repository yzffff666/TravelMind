...
def create_multi_tool_workflow(
    ...
    # 添加边
    # 将guardrails的输出作为planner的输入
    main_graph_builder.add_edge(START, "guardrails")

    # 如果guardrails的输出为planner，则路由到planner
    main_graph_builder.add_conditional_edges(
        "guardrails",
        guardrails_conditional_edge,
    )

    # 将planner的输出作为tool_selection的输入
    main_graph_builder.add_conditional_edges(
        "planner",
    ...
    # 将cypher_query、predefined_cypher、customer_tools的输出作为summarize的输入
