...
from app.services.destination_grounding import DestinationResolver, filter_candidates_for_destination
...
async def probe_cases(cases: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for case in cases:
        ...
        if profile.resolved:
            ...
            result.update(
                {
                    "status": "ready" if len(accepted) >= 3 else "insufficient_candidates",
                    "validated_candidate_count": len(accepted),
                    "provider_candidate_count": len(recall.candidates),
                    "provider_degraded": recall.degraded,
    ...
    return {
        "schema_version": "live_destination_grounding_probe_v1",
