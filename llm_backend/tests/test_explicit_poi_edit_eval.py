from scripts.explicit_poi_edit_eval import DEFAULT_CASES_PATH, evaluate_cases, is_passing, load_cases


def test_explicit_poi_edit_eval_passes_default_cases():
    summary = evaluate_cases(load_cases(DEFAULT_CASES_PATH))

    assert summary["case_count"] >= 16
    assert summary["explicit_cases"] >= 7
