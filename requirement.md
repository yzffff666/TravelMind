...
> **技术实现映射**：上述产品级字段在技术实现中对应 `EvidenceItem`（`app/schemas/itinerary_v1.py`），字段名为 `evidence_id` / `provider` / `title` / `url` / `snippet` / `fetched_at` / `attribution`。详细映射见 `design.md` 6.1 节与 `docs/下层能力流水线技术方案.md` 4.5 节。
