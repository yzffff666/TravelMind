...
### 4.1 QP（Query Processing）— 规则 baseline + Structured QP 增强
...
- 实现方式：
  - 默认路径：规则 baseline（正则 + 关键词匹配 + `qp_rules` 配置），保证低成本、可解释、可回退。
  - 增强路径：`Structured QP`（`structured_qp.py`），通过 LLM Structured Output 做意图识别、约束抽取、query rewrite 和置信度判断。
  - 开关策略：`ENABLE_STRUCTURED_QP=false` 默认关闭；开启后若超时、异常、JSON 校验失败或置信度低于阈值，则回退规则 baseline。
...
QPOutput:
  ...
  qp_source: str               # rule | llm | fallback
  confidence: float | None
  fallback_reason: str | None
```

**增强路线**（对齐 `task.md` T-M2-009a）：

| 方向 | 技术 | 说明 |
|------|------|------|
| LLM Structured QP | Structured Output（JSON mode）+ Pydantic 校验 | 已落地 MVP；默认关闭，可通过 feature flag 灰度，失败回退规则 baseline |
| BERT 意图分类 | 轻量分类模型，可插拔 | 中期方案；待积累真实中文 query 与 golden labels 后再评估 |
| Query 改写 | LLM rewrite | 已作为 Structured QP 输出字段保留，后续可用于召回 query 扩展 |

**面试说法**：M1/M2 用规则 baseline 保证可控可解释；随后补充 Structured QP 模型增强，用 feature flag、置信度阈值、Pydantic schema 和 fallback 保证主链路稳定。这样既利用 LLM 处理模糊表达和多轮上下文，又不让模型单点接管路由。
