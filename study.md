...
- 理解下层搜索能力链路：QP -> Recall -> Ranking -> Rule Filter -> Evidence Builder（技术选型与大厂对比见 docs/下层能力流水线技术方案.md）。
- 理解 QP 的定位：给召回/排序提供结构化输入，而不是替代 Agent 控制面。
- 理解召回技术选型：外部搜索 API（SearchProvider / MapProvider）+ 多路去重聚合 + mock 降级，不自建 POI 库。
- 理解排序技术选型：可解释规则打分（加权多维：偏好/预算/评分/热度/证据质量），而非 CTR 模型。
- 理解为什么旅行场景下规则过滤优先级高（预算/时间/闭馆/通勤等硬约束），采用链式规则过滤器。
...
- 理解前端 UX 对齐 Agent 产品的核心原则：双栏联动、SSE 驱动状态流、骨架屏/流式渲染、空态/异常态兆底。

### 推荐任务顺序（已对齐 task.md 第 3 节建议执行顺序）

**第一阶段：对话式连续编辑闭环**（对应 task.md 第 4 步）
1. `T-M2-000a`（QP baseline：意图识别 + 约束抽取）
2. `T-M2-010`（会话级 itinerary 状态持久化）
3. `T-M2-011`（对话意图路由：create/edit/qa/reset）
4. `T-M2-012`（Patch 编辑引擎）
5. `T-M2-013`（SSE 扩展：编辑结果与差异事件）
6. `T-M2-014`（前端对话+行程同屏联动）

**第二阶段：前端交互骨架与体验**（对应 task.md 第 5 步）
7. `T-M2-015`（前端交互骨架：双栏联动 + 状态反馈 + 空态/异常态）
8. `T-M2-016`（P1：流式体验与视觉精修）
9. `T-M2-017`（P1：响应式适配与无障碍基础）

**第三阶段：Provider + 搜索链路 + 证据**（对应 task.md 第 6 步）
10. `T-M2-001`（Provider 抽象接口定义）
11. `T-M2-002a`（策略配置与调用上限 N）
12. `T-M2-002b`（降级路径与 assumptions/risk 提升规则）
13. `T-M2-006`（Provider mock/fixture，离线可跑）
14. `T-M2-000b`（候选召回 baseline：外部搜索 API + 多路聚合）
15. `T-M2-000c`（可解释排序 baseline：规则打分）
16. `T-M2-000d`（规则过滤 baseline：硬约束过滤）
17. `T-M2-003`（Evidence v1 结构与归因落地）
18. `T-M2-000e`（证据组织与映射 baseline：Evidence Builder）
19. `T-M2-004`（证据覆盖率统计口径实现）
20. `T-M2-005`（P1：前端证据面板与关联展示）

**第四阶段：可选增强**（对应 task.md 第 7 步，触发条件满足后）
21. `T-M2-002c`（P1：条件并行）
...
**第一阶段：对话式连续编辑闭环**
1. `T-M2-000a`：QP 输出如何统一上游 query 与下游召回输入；意图识别如何区分首次生成/编辑/问答。
2. `T-M2-010`：会话级 itinerary 状态为什么是对话共创的前提。
3. `T-M2-011`：create/edit/qa/reset 路由如何避免“每轮整单重算”。
4. `T-M2-012`：patch 操作语义与 apply 后校验。
5. `T-M2-013`：diff/revision 如何通过 SSE 回传并保证兼容。
6. `T-M2-014`：聊天区与行程区同屏联动的状态管理方法。

**第二阶段：前端交互骨架与体验**
7. `T-M2-015`：为什么 Agent 产品采用双栏联动（Chat + Panel）而非单栏；SSE 事件如何映射到组件状态机（Empty/Clarifying/Streaming/Complete/Error）；EmptyState 和 ErrorState 对用户留存的影响。
8. `T-M2-016`：骨架屏 vs spinner 对感知延迟的差异；流式逐字渲染的实现（SSE chunk -> DOM 增量）；证据抽屉交互模式参考（Perplexity sidebar）；动效克制原则（仅状态切换用过渡，不加装饰性动画）。
9. `T-M2-017`：CSS Container Queries / Media Queries 的响应式断点策略；键盘导航与焦点管理（Tab order、focus trap）；WCAG 2.1 AA 级对比度要求及检查工具。

**第三阶段：Provider + 搜索链路 + 证据**
10. `T-M2-001`：抽象接口如何防止业务层绑定具体 provider。
11. `T-M2-002a`：配置化策略与调用上限 N 的治理思路。
12. `T-M2-002b`：assumptions/risk 作为降级可解释出口。
13. `T-M2-006`：离线 fixture 对开发效率与回归稳定性的意义。
14. `T-M2-000b`：通过 SearchProvider / MapProvider 调用外部搜索 API 召回候选，多路去重聚合的工程边界（覆盖优先、可回退、mock 降级）。技术选型见 docs/下层能力流水线技术方案.md 4.2 节。
15. `T-M2-000c`：可解释规则打分与排序可观测性。
16. `T-M2-000d`：硬约束过滤如何保障可执行性与安全性。
17. `T-M2-003`：evidence 字段、归因、跳转与合规边界。
18. `T-M2-000e`：候选到 evidence/refs 的映射与可追溯性。
19. `T-M2-004`：覆盖率口径（slot 主 POI）和统计边界。
20. `T-M2-005`：前端证据卡片与推荐项关联的体验设计。
...
### 第 3 周（M2 第一阶段起步：QP + 会话态 + 意图路由）
- 完成：`T-M1-006`、`T-M2-000a`、`T-M2-010`、`T-M2-011`
- 目标：打通“QP 统一输出 + 会话态持久化 + 意图路由”地基（对话共创前置条件）

### 第 4 周（M2 收口 + 前端骨架）
- 完成：`T-M2-012`、`T-M2-013`、`T-M2-014`、`T-M2-015`
- 目标：完成“patch 编辑 + diff SSE + 前端同屏联动”最小闭环

### 第 5 周（M2 第三阶段：Provider + 召回/排序/过滤流水线）
- 完成：`T-M2-001`、`T-M2-002a`、`T-M2-002b`、`T-M2-006`、`T-M2-000b`、`T-M2-000c`、`T-M2-000d`
- 目标：Provider 抽象接口 + 调用策略与降级路径；打通召回→排序→规则过滤流水线（技术选型见 `docs/下层能力流水线技术方案.md`）

### 第 6 周（M2 Evidence 收口 + 前端精修）
- 完成：`T-M2-003`、`T-M2-000e`、`T-M2-004`
- 可选 P1：`T-M2-005`、`T-M2-016`（流式体验与视觉精修）、`T-M2-017`（响应式与无障碍）
- 目标：Evidence 链路闭环；前端体验从"能用"进阶到"好用"

### 第 7 周（M3 起步 + DeepAgents/DeepResearch 评估窗口）
