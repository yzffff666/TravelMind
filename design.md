...
flowchart LR
    ...
    subgraph BE["Backend Layer (`llm_backend`)"]
...
> **实现现状注记**：当前代码（`query_processor.py` `QPOutput`）输出为单条 `recall_query: str`（拼接原始查询 + 结构化约束），尚未扩展为多条 `recall_queries: list[str]`。上述示例为设计目标态，后续可按需扩展。技术选型详见 `docs/下层能力流水线技术方案.md` 4.1 节。
